...
class Tweet
{
	constructor(rawData = {})
	{
		 ...
		 * @type {String}
		 */
		this.id = rawData.data.id;
		/**
		 * @type {String}
		 */
		this.text = rawData.data.text;
		this.createdAt = new Date(rawData.data.created_at);
		this.includes = rawData.includes;
		/**
		 * @type {Array<Object>}
		 */
		this.rules = rawData.matching_rules;

		if (rawData.includes.users && rawData.includes.users.length)
			this.author = rawData.includes.users[0];
	}
}
...
class TwitterRules
{
	...
	constructor(rulesURL = "https://api.twitter.com/2/tweets/search/stream/rules", credentials = {})
	{
		...
		if (!fs.existsSync(TwitterRules.PATH_RULES))
			fs.outputJSON(TwitterRules.PATH_RULES, {});
	...
	validate()
	{
		this.fetch()
			.then(rules =>
			{
				fs.readJSON(TwitterRules.PATH_RULES)
					.then(tags =>
					{
						const to = {
							create: [],
...
module.exports = { Tweet, TwitterCredentials, TwitterRules, TwitterStream };
