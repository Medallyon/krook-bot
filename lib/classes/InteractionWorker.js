...
class InteractionWorker
{
	...
	constructor(client)
	{
		...
		this.workQueue.process(InteractionWorker.MAX_JOBS_PER_WORKER, async (job, done) =>
		{
			...
			if (cmd == null)
				return done(new Error(`Command '${cInteraction.data.name}' does not exist`));

			cmd.run(cInteraction)
				.then(response =>
				{
					const body = new WebInteractionResponse(cmd.type, response);

					console.log("\nfollowing up with", body);
					this.request(`/webhooks/${process.env.DISCORD_CLIENT_ID}/${cInteraction.token}/callback`, { body }, done);
				}).catch(done);
		});
	}
}
